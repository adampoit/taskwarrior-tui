...
`taskwarrior-tui` [`--profile NAME`] [`--report STRING`]
...
`-p`, `--profile NAME`
: Select a configured Taskwarrior profile. This option conflicts with `--taskrc` and `--taskdata`.

`--list-profiles`
: List configured profile names, labels, and the default profile, then exit.

`--taskrc FILE`
: Set the Taskwarrior configuration file through `TASKRC` when no profile is selected.

`--taskdata FOLDER`
: Set the Taskwarrior data directory through `TASKDATA` when no profile is selected.

`-c`, `--config FOLDER`
: Set the taskwarrior-tui configuration directory containing `profiles.toml`.

`-d`, `--data FOLDER`
: Set the taskwarrior-tui data directory used for logs and history.
...
: Start with the configured `work` profile. Profiles select isolated Taskwarrior databases; they do not filter synchronization by project.
...
`p`
: profile switcher menu - Open profile switcher menu
...
`j`
: {selected+=1} - Move forward a profile

`k`
: {selected-=1} - Move back a profile
